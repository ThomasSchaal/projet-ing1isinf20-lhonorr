...
public class Prim {
	...
	public static Result_list do_Prim(int[][] tab) {
		...
		while(totalNbNodes(dif_nodes) != Graph.getNbNodes() ) {
			...
			while(cycle_found) {
				// On récupère l'arête minimale de la liste des sommets à check via la méthode look_neightbour
				minimal_node = Prim.look_neighbour(tab, checked_node);
	 ...
	 * @return
	 */
	public static int[] look_neighbour(int[][] tab, List<Integer> checked_node) {
